...
@EndOfDesignText@
'version 2.00
#DesignerProperty: Key: CurrentValue, DisplayName: Current Value, FieldType: Float, DefaultValue: 0
...
#DesignerProperty: Key: TextSize, DisplayName: Text Size, FieldType: Float, DefaultValue: 10
...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	mCurrentValue=Props.Get("CurrentValue")
	PrefixText = Props.Get("PrefixText")
	SuffixText = Props.Get("SuffixText")
	DurationFromZeroTo100 = Props.Get("Duration")
	textcolor= xui.PaintOrColorToColor(Props.GetDefault("TextColor", xui.Color_White))
	...
	mlbl = Lbl
	mlbl.Font = xui.CreateFont( NativeFont, textsize)
	mlbl.SetTextAlignment("CENTER", "CENTER")
	mlbl.TextColor =textcolor
...
Private Sub DrawBackground
	...
	If GaugeType = FULL_CIRCLE And PrefixText <> "" Then
		cvsGauge.DrawText(PrefixText, x, y - 30dip, xui.CreateDefaultFont(16), textcolor, "CENTER")
		'outer border
	End If
	cvsGauge.DrawCircle(x, y, Radius + 1dip, CenterColor, False,  1dip)
...
Private Sub ConfigureFullCircle
	...
	AngleOffset = 135
	AngleRange = 270
	IndicatorBaseWidth = 8dip
End Sub

Private Sub DrawTicks1
...
Private Sub DrawIndicator (Value As Float)
	...
	Dim p As B4XPath
	p.Initialize( x + (IndicatorBaseWidth-4dip) * SinD(angle), y - (IndicatorBaseWidth-4dip) * CosD(angle))
	Dim length As Float = Radius - 0.3 * IndicatorLength
	p.LineTo(x + length * CosD(angle), y + length * SinD(angle) )
	p.LineTo(x - (IndicatorBaseWidth-4dip) * SinD(angle), y + (IndicatorBaseWidth-4dip) * CosD(angle))
	...
	For i = 0 To NumberOfTicks
		...
		If i Mod 5 = 0 Then
			...
			cvsGauge.DrawTextRotated(NumberFormat(MinValue + i / NumberOfTicks * (MaxValue - MinValue), 1, 0), _
               x + tr * CosD(angle), y + tr * SinD(angle), xui.CreateDefaultFont(10), textcolor, "CENTER", angle + 90)
		Else
			thickness = 1dip
			r = ShortTick
		End If
		cvsGauge.DrawLine(x + r * CosD(angle), y + r * SinD(angle), x + r1 * CosD(angle), y + r1 * SinD(angle), textcolor, thickness)
	Next
End Sub